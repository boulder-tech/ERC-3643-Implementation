...
//     signer).deploy(implementationAuthority,
//        managementKey,);
...
// async function obtenerGasLimitAlgunaManera(): Promise<number> {
//   // En este ejemplo, se devuelve un valor fijo de gas limit
//   return 5000000;
// }

export async function deployIdentityProxy(implementationAuthority: Contract["address"], managementKey: string, signer: Signer) {
  try {
    // Obtener el límite de gas antes de desplegar el contrato
    const gasLimit = 25000000 //await obtenerGasLimitAlgunaManera();

    const paramsIdentityProxy = [
      implementationAuthority,
      managementKey
    ]
    // Desplegar el contrato con el gasLimit ajustado
    const identity = await new ethers.ContractFactory(
      OnchainID.contracts.IdentityProxy.abi, 
      OnchainID.contracts.IdentityProxy.bytecode, 
      signer
    ).deploy(...paramsIdentityProxy, {
      gasLimit: gasLimit,
    });

    // Obtener una instancia del contrato desplegado
    return ethers.getContractAt("Identity", identity.address, signer);
  } catch (error) {
    console.error("Error al desplegar el contrato IdentityProxy:", error);
